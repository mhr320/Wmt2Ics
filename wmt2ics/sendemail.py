...
class SendEmail:
	...
	def send_mail(self):
		print("Attching " + self.outfile.split('/')[-1] + " to Email, sending...")
		...
		self.server = smtplib.SMTP(self.cfg["SMTPSERVER"], int(self.cfg["SMTPPORT"]))
		self.server.starttls()
		self.server.login(self.cfg["SENDER"], self.cfg["APP_PWD"])
		self.server.sendmail(self.cfg["SENDER"], self.cfg["RECEIVER"], self.text)
		self.server.quit()
